...
def main():
    ...
    ax1.set_title("KTP Crystal Refractive Index vs Wavelength")
    ...
    ax2.set_ylabel("Phase Matching Angle φ (degrees)")
    ax2.set_ylim(0, 90)
    ax2.set_title("Phase Matching Angle φ vs Wavelength")
    ax2.grid(True)
    st.pyplot(fig2)
    ...
    ax3.plot(phi_arr, eta_arr, 'b-', label="Conversion Efficiency")
    ax3.axvline(opt_phi, color='r', linestyle='--', label=f"Optimal Angle: {opt_phi:.2f}°")
    ax3.set_xlabel("Azimuth Angle φ (degrees)")
    ax3.set_ylabel("Relative Output Power")
    ax3.set_ylim(0, 1.05)
    ax3.grid(True)
    ax3.legend()
    st.pyplot(fig3)
